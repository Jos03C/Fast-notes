...
\begin{thmbox}
	\begin{theorem}[Rank]\label{thm:mat_rank}
...
\begin{itemize}
	\item $A$ full column rank $\rmr(A)=m$, then $A^\TT A$ nonsingular, and $A_{left}^{-1}=(A^\top A)^{-1}A^\top$;
	\item $A$ full row rank $\rmr(A)=n$, then $AA^T$ nonsingular, and $A_{right}^{-1}=A^{\top}(AA^{\top})^{-1}$.
\end{itemize}
...
\begin{exbox}
	\begin{example}[M-P inverse of a diagonal matrix]
