...
If $A$ is full rank, then we use 
\begin{itemize}
	\item \red{left inverse} if $n>m$: $A_{left}^{-1}=(A^\top A)^{-1}A^\top$, i.e., $A_{left}^{-1}A=I_m$
	\item \red{right inverse} if $n<m$: $A_{right}^{-1}=A^{\top}(AA^{\top})^{-1}$, i.e., $A A_{right}^{-1}=I_n$. 
\end{itemize}
