...
\begin{thmbox}
	\begin{theorem}[Column spaces and rank]\label{thm:mat_colsp}
...
\subsection{Matrix inverse}\label{sec:mat_inverse}
\begin{thmbox}
	\begin{theorem}[Push-through identity]\label{thm:push-through_id}
		Let $U\in\bbR^{n\times k}, V\in\bbR^{k\times n}$, then 
		\begin{equation}\label{eq:push-through_id}
			(I_n+UV)^{-1}U=U(I_n+VU)^{-1}.
		\end{equation} 
	\end{theorem}
\end{thmbox}
...
\begin{thmbox}
	\begin{theorem}[\href{https://en.wikipedia.org/wiki/Woodbury_matrix_identity}{Sherman--Morrison--Woodbury formula/matrix inversion lemma}]\label{thm:SMW_inv}
...
\begin{thmbox}
	\begin{theorem}\label{thm:idempotent}\
		\begin{enumerate}
			...
			\item If $A$ is idempotent, then $\rmr(A)=\tr(A)$, and $\lambda(A)$ is either 0 or 1. 
			\item If $A$ symmetric, all $\lambda(A)$'s are 0 or 1, then $A$ is idempotent. 
			\item If $A$ and $V$ are symmetric and $V\succ\0$, then $AV$ has eigenvalues 0 and 1 implies that $AV$ is idempotent.     
		\end{enumerate}
	\end{theorem}
\end{thmbox}
...
\begin{defbox}
	...
	If $M$ satisfies $M\v=\v$, $\forall \v\in\Col(M)$, then $M$ is called the \red{projection operator (matrix)} onto $\Col(M)$ along $\ker(M)$.
	Any projection operator that is not p.p.m. is called an \red{oblique projection operator}.
	\end{definition}
\end{defbox}
...
\begin{thmbox}
	\begin{theorem}[Relationships between two p.p.m. (Thm B.45--49 \cite{christensen2002plane})]\label{thm:ppm_two}
