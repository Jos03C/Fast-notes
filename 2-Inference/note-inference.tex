...
Reference: 
\begin{itemize}
	\item Robb J. Muirhead - Aspects of multivariate statistical theory \cite{muirhead2009aspects}.
...
\begin{thmbox}
	...
	\end{theorem}
\end{thmbox}
The proof uses the fact that a distribution in $\bbR^p$ is uniquely determined by its ch.f., see Theorem 1.2.2. \cite{muirhead2009aspects}.
...
\begin{defbox}
	\begin{definition}\label{def:sample_mean_etc}
		...
		\begin{itemize}
			...
			\item (Sum of squares and cross product (SSCP) matrix) $A=\sum_{k=1}^{n}(\x_k-\bar{\x})(\x_k-\bar{\x})^\TT$.
...
\begin{itemize}
	...
	\item (\red{Representation}) Let $\Sigma=HDH^\TT$ be the spectral
