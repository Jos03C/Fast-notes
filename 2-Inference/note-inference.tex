...
\begin{salign*}
	...
	(\text{Under $H_0$}) \qquad \SSE_{H_0}-\SSE&=Q.
\end{salign*}
...
\begin{salign*}
	...
	&=\Var(\hat{\bab})-\underbrace{\sigma^2 (\X^\TT\X)^{-1}\K(\K^\top(\X^\top \X)^{-1}\K)^{-1}\K^\TT(\X^\TT\X)^{-1}}_{\succeq \0}.
\end{salign*}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{salign*}
				...
				\end{pmatrix}^{-1}
				=\begin{pmatrix}
					\frac{1}{n}+\bar{\X}^\TT(\X_c\X_c)^{-1}\bar{\X} & -\bar{\X}^\TT (\X_c\X_c)^{-1} \\ -(\X_c\X_c)^{-1}\bar{\X} & (\X_c\X_c)^{-1}
				\end{pmatrix}.
			\end{salign*}
			Then 
			\begin{salign*}
				(\X^\TT\X)^{-1}\X^\TT &= \begin{pmatrix}
					\frac{1}{n}\1_n^\TT + \bar{\X}^\TT (\X_c\X_c)^{-1}\bar{\X}\1_n^\TT -\bar{\X}^\TT (\X_c\X_c)^{-1} \X_1^\TT \\
					-(\X_c\X_c)^{-1}\bar{\X}\1_n^\TT + (\X_c\X_c)^{-1} \X_1^\TT
				\end{pmatrix},\\
				H=\X(\X^\TT\X)^{-1}\X^\TT &=\frac{1}{n}\1_n\1_n^\TT-(\X_1-\1_n\bar{\X}^\TT)(\X_c\X_c)^{-1}\bar{\X}\1_n^\TT + (\X_1-\1_n\bar{\X}^\TT)(\X_c\X_c)^{-1}\X_1^\TT\\
...
\begin{proof}
	...
	\begin{salign*}
		Q&=({\K^\top\hat{\bab}}-\m)^\top[\K^\top(\X^\top \X)^{-1}\K]^{-1}({\K^\top\hat{\bab}}-\m)\\
		&=({\K^\top(\X^\top \X)^{-1}\X^\top \Y}-\m)^\top[\K^\top(\X^\top \X)^{-1}\K]^{-1}({\K^\top(\X^\top \X)^{-1}\X^\top \Y}-\m),
	\end{salign*}  
	But 
	\begin{salign*}
	...
	Therefore,
	\begin{salign*}
		Q&=(\Y-\X\K(\K^\top \K)^{-1}\m)^\top \X(\X^\top \X)^{-1}\K(\K^\top(\X^\top \X)^{-1}\K)^{-1}\K^\top(\X^\top \X)^{-1}\X^\top\underbrace{(\Y-\X\K(\K^\top \K)^{-1}\m)}_{\d}\\
		&=\d^\TT [\X(\X^\top \X)^{-1}\K(\K^\top(\X^\top \X)^{-1}\K)^{-1}\K^\top(\X^\top \X)^{-1}\X^\top] \d =: \d^\TT \B\d.
	\end{salign*}
	On the other hand,
	\begin{salign*}
		\SSE &= \Y^{\top}(I_n-\X(\X^{\top}\X)^{-1}\X^{\top})\Y \\
		&=(\boldsymbol{Y}-\boldsymbol{X}\boldsymbol{K}(\boldsymbol{K}^{\top}\boldsymbol{K})^{-1}\m)^{\top}(I_n-\boldsymbol{X}(\boldsymbol{X}^{\top}\boldsymbol{X})^{-1}\boldsymbol{X}^{\top})(\boldsymbol{Y}-\boldsymbol{X}\boldsymbol{K}(\boldsymbol{K}^{\top}\boldsymbol{K})^{-1}\m)\\
		&=\d^\top(I-\X(\X^\top \X)^{-1}\X^\top)\d =: \d^\TT \A \d,
	\end{salign*}
	because $\X^\top(I-\X(\X^\top \X)^{-1}\X^\top)=\0$ and $(I-\X(\X^\top \X)^{-1}\X^\top)\X=\0$.
	Since 
	\begin{sequation*}
		\A\Sigma \B=(I-\X(\X^\top \X)^{-1}\X^\top)^\top\sigma^2 I\X(\X^\top \X)^{-1}\K[\K^\top(\X^\top \X)^{-1}\K]^{-1}\K^\top(\X^\top \X)^{-1}\X^\top=\0,
	\end{sequation*}
	by Theorem \ref{thm:MVN_quad_indep}, $Q\indep \SSE$. 
	Then since 
	\begin{sequation*}
		\frac{(\K^\top(\X^\top \X)^{-1}\K)^{-1}}{\sigma^2}(\K^\top(\X^\top \X)^{-1}\K)\sigma^2=I_s,
	\end{sequation*}
