...
\begin{defbox}
	\begin{definition}\label{def:random_vector_moments}
		...
		\begin{itemize}
			\item \red{Mean} $\bbE \x = \bmu=(\bbE x_1,\ldots,\bbE x_p)^\TT=(\mu_j)$.
...
\begin{itemize}
	...
	\begin{sequation*}
		\begin{aligned}
			\A&= \sum_{i=1}^{n}(\x_i-\bmu)(\x_i-\bmu)^\TT-n(\bar{\x}-\bmu)(\bar{\x}-\bmu)^\TT \\ & =(\X-\1_n\bar{\x}^\TT)^\TT(\X-\1_n\bar{\x}^\TT)\succeq \0.
		\end{aligned}
	\end{sequation*}
...
\begin{thmbox}
	\begin{theorem}[Properties of multivariate normal]\label{thm:multi_normal_prop}
		...
		\begin{enumerate}
			...
			\item (\red{Quadratic form})  If $\x_{1:n}\simiid \rmN_p(\bmu,\Sigma)$, then $n(\bar{\x}-\bmu)^{\TT}\Sigma^{-1}(\bar{\x}-\bmu)\sim\chi_p^2$. The squared generalized distance (Mahalanobis distance) 
...
\begin{thmbox}
	\begin{theorem}[CLT for sample covariance matrices]\label{thm:CLT_multi_sampleCov_iid}
		Let $\x_{1:n}\simiid[\bmu,\Sigma]$ with finite fourth moments, SSCP matrix $\A=\sum_{i=1}^{n}(\x_i-\bar{\x})(\x_i-\bar{\x})^\TT$, and $\S=(n-1)^{-1}\A$. Let $V=\Cov[\vec((\x_1-\bmu)(\x_1-\bmu)^\TT)]$, then 
		\begin{sequation*}
			\begin{aligned}
