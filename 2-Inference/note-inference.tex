	...
	\begin{proposition}\label{prop:post_linearSEM}
		...
		\begin{enumerate} 
			...
			\begin{equation*}
				...
				\end{aligned}
			\end{equation*}
			\item Assume all elements of $\baL_k$ and $\baL_\bao$ are unknown, conditional distribution $\red{\bbP(\baq\mid\Y,\baO)}=\bbP(\baq_\y\mid\Y,\baO)\bbP(\baq_\bao\mid\Y,\baO)$, where 
			\begin{sequation*}
				\begin{aligned}
					\bbP(\baq_\y\mid\Y,\baO)&\propto \prod_{k=1}^{p} \bbP(\baL_k,\psi_{\bae k}\mid \Y,\baO),\\
					\bbP(\baq_\bao\mid\Y,\baO)&\propto \mbk{\prod_{k=1}^{q_1}\bbP(\baL_{\bao k},\psi_{\delta k}\mid \baO)}\bbP(\baF\mid\baO_2)
				\end{aligned}
			\end{sequation*} 
			where 
			\begin{sequation*}
				\begin{aligned}
					...
					\bbP(\baL_{\bao k},\psi_{\delta k}^{-1}\mid \baO)&\propto\rmN_{r_2+q_1+q_2}(\a_{\bao k},\psi_{\delta k}\A_{\bao k})\cdot\Ga(n/2+\alpha_{0\delta k}, \beta_{\delta k}),\\
			...
			\begin{equation*}
				\begin{aligned}
					...
					\beta_{\delta k}&=\beta_{0\delta k}+\frac{1}{2}(\baX_{k}^{\TT}\baX_{k}-\a_{\bao k}^{\TT}\A_{\bao k}^{-1}\a_{\bao k}+\baL_{0\bao k}^{\TT}\H_{0\bao k}^{-1}\baL_{0\bao k}).
				\end{aligned}
			\end{equation*}
		\end{enumerate}
	\end{proposition}
